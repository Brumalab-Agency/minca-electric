...
function Notification({ message, subMessage, show, onClose, error }) {
  ...
  return (
    <div
      className={`fixed right-0 top-20 z-50 m-4 mt-10 flex items-center space-x-2 rounded-md p-4 shadow-md ${error ? "bg-zinc-950 text-white" : "bg-zinc-950 text-white"}`}
    >
      <svg
        width="20"
        height="20"
      ...
      <div>
        <span
          className={`mr-5 font-semibold ${error ? "text-white" : "text-white"}`}
        >
          {message}
        </span>
        <div className="mr-5 text-sm">{subMessage}</div>
      </div>
...
function Newsletter() {
  ...
  const handleSubmit = async (event) => {
    ...
    setShowNotification(true);
    setSuccessMessage("Te has suscrito con éxito");
    setIsError(false);
    setTimeout(() => {
      setShowNotification(false);
      setSuccessMessage("");
  ...
  return (
    <div>
      <form onSubmit={handleSubmit}>
        ...
        <p
          className={`text-[#22BB55] ${successMessage ? "mt-3 text-[#22BB55]" : ""}`}
        >
          {successMessage || " "}
        </p>
      </form>
