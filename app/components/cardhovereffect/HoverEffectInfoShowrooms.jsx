    ...
    >
      {items.map((item, idx) => (
        <Link
          href={item?.link}
          key={item?.link}
          className="group relative block  h-[418px] w-full rounded-[10px] p-2 lg:h-[402px] lg:w-[402px] "
        ...
        >
          <AnimatePresence>
            {hoveredIndex === idx && (
              <motion.span
                className="absolute inset-0 block h-full w-full rounded-3xl bg-[#FF3333] dark:bg-[#878D8F]"
...
export const Card = ({
  ...
  return (
    <div
      className={cn(
        "rounded-2xl h-full w-full p-4 overflow-hidden bg-[#111111] border-transparent dark:border-white/[0.2] group-hover:border-slate-700 relative z-20 border-white border-2",
        className
      )}
    >
